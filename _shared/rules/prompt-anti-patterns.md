...
### Lesson 2026-05-03 — TNTD030426i772 pregnancy announcement wooden keepsake (multi-iteration session)

See full case study: `_shared/xp/case-studies/TNTD030426i772-pregnancy-announcement-wooden-keepsake.md`

#### Operational credit/versioning rules (hard rules from Boss)

16. **ONE-SUBMIT per paid generation task.** For both openai/gpt-image-2 and KIE, submit the task once, wait for completion, save the output, and report. Never self-submit a new task because the first output is slow, incomplete-looking, or visually not ideal. Every extra task burns real money.
17. **Immediate version preservation.** After every successful paid generation, copy outputs to a versioned folder (`v##_provider`) before starting another backend or another regen run. Canonical output files may be overwritten by the runtime. Losing good outputs because of overwrite is a preventable error.
18. **No parallel credit-burning without explicit Boss authorization.** Generating multiple slots/modules in parallel counts credits fast. Unless Boss explicitly says "batch" or "all slots", submit one at a time.

#### Technical lessons

19. **Text-heavy engraved/personalized products require a stricter fidelity gate.** Visual beauty does not equal pass. Pass requires: key text readable, product mechanism preserved, no added/removed text on product face, upright orientation, no warping if product is rigid.
20. **nano-banana-pro remains the stable KIE fallback for fidelity.** seedream-4 returned 422 on this account (model not supported). gpt-image-2-image-to-image via KIE generated successfully but sometimes produced non-square native dimensions that were then force-resized. For maximum fidelity, nano-banana-pro is the single reliable KIE fallback until otherwise configured.
21. **Close-up lifestyle/detail shots are often safer than complex people scenes** for small text-heavy keepsakes. People scenes risk: hands cover message, product bends like paper, product becomes too small. Close-up detail (slot 8 style) preserves both text and two-piece structure better.
22. **Prompt for scene, not product anatomy.** For engraved products, long product-structure descriptions cause the model to rebuild from text; short scene-only prompts with reference-image product anchoring produce better fidelity (same root cause as Lesson 2026-04-11 text dominance).
...
### RULE-003: For any product where an insert is **visible through** a cut
...
- **Verified:** 3 times (avg score: 6.8)
- **Source:** XP-003, XP-184, XP-141
- **Categories:** errors, slots

### RULE-004: For use-case shots with hand + insertion action, the model i
...
- **Verified:** 3 times (avg score: 6)
- **Source:** XP-001, XP-178, XP-142
- **Categories:** errors, slots

### RULE-005: Persona quote → arrow → thumbnail → product name flow works 
- **Rule:** Persona quote → arrow → thumbnail → product name flow works for decision-guide cards. Quotes should be 2 lines max for readability at A+ banner scale
- **Verified:** 3 times (avg score: 8.4)
- **Source:** XP-228, XP-188, XP-121
- **Categories:** slots

### RULE-006: Infographic with 3 detail circles but NO text labels — AI ca
- **Rule:** Infographic with 3 detail circles but NO text labels — AI cannot gen accurate text. Recommend post-process text annotations for conversion.
- **Verified:** 3 times (avg score: 8.6)
- **Source:** XP-203, XP-154, XP-123
- **Categories:** slots

### RULE-007: For slot1 hero, using a pure-white studio prompt with explic
