...
export default function Shell() {
  ...
  // Last per-slot generation failure reason (transient — cleared when the
  // operator starts a new generate for that slot).
  const [slotGenErrors, setSlotGenErrors] = useState({})
  // Banner shown when a real-provider Generate falls back to Mock because
  // the key is missing. 5s auto-clear.
  ...
  const chooseSku = useCallback(async (target) => {
    ...
    // entries that are about to be deleted.
    setSlotTmpImages({})
    setSlotGenErrors({})
    cleanupExpired({ skuPath: target })
      .then(() => loadLatestPerSlot({ skuPath: target }))
      .then((tmp) => setSlotTmpImages(tmp))
      .catch((err) => console.error('[tmp-cache] load failed', err))
    if (!api) return
    setValidating(true)
    try {
  ...
  // (Generate / Cancel dispatcher inserted below — see generateSlot.)
  ...
  const refreshTmpImages = useCallback(async () => {
    if (!skuPath) return
    const tmp = await loadLatestPerSlot({ skuPath })
  ...
  const generateSlot = useCallback(async (slotId) => {
    ...
    // router the real runtime + mock pipeline use. The canonical state
    // machine reads these lines and lights up the slot.
    ...
    handlePipelineEvent({
      kind: 'log',
      stream: 'stdout',
      line: `[Slot ${slotId}] starting generation`,
      ts: startTs + 1
    })
  ...
  // current SKU if one is already loaded from localStorage state.
  useEffect(() => {
    if (skuPath) cleanupExpired({ skuPath }).catch(() => {})
    // intentionally empty deps — startup only
      ...
      <section className="shell__main">
        ...
        <div className="shell__canvas">
          <MainCanvas
            ...
            onCancelSlotGeneration={cancelSlotGeneration}
